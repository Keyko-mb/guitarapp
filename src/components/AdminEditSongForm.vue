...
export default {
  ...
  mounted() {
    ...
    axios
        .get("http://localhost:8084/api/songId/" + this.songToEdit.uuid)
        .then((response) => {
          this.song.name = response.data.name
          this.song.text = response.data.text
          this.song.author = this.songToEdit.author
        })
    axios
        .get("http://localhost:8084/api/song/" + this.songToEdit.uuid + "/accords")
        .then((response) => {
          this.song.accords = response.data
        })
  },
  computed: {
    sortedAuthors() {
